...
csv_file = Path('/home/remi/Documents/sherbrooke_citoyen/training_img_15oct/bingo_trn.csv')
...
with open(csv_file, 'r') as f:
    ...
    for index, row in tqdm(enumerate(reader)):
        ...
        file = Path(f"{row[0]}")  # add ".png" is a work around because csv is not well written
        # check file's existence
        if not file.is_file():
            raise FileNotFoundError(f'Not found: {file}')
        # copy to destination folder
        if not (out_img_dir_dataset / file.name).is_file():
            shutil.copy(file, out_img_dir_dataset)
            #print(f'Copied: {file}')
        ...
        if not lbl_file.is_file():
            with open(str(lbl_file), 'w') as out_lbl:
                if row[1] is not '':
